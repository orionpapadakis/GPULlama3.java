...
/**
 * Builds the {@code InferenceProgram} for Phi3, single-token, at a given weight representation.
 *
 * <ul>
 *   <li><b>Attention:</b> one {@code MatVec} of width {@code dim + 2·kvHeads·headSize} against the
 *       {@code ATTENTION_QKV} matrix, then {@link SplitFusedQkv}.
 *   <li><b>Feed-forward:</b> one {@code MatVec} of width {@code 2·hiddenDim} against the fused
 *       gate/up matrix, then {@link SplitGateUp}, then the ordinary {@link SwiGLU}.
 * </ul>
 *
 * <p>Each fused projection stays <b>one</b> {@code MatVec}, because that is what the model
 * performs; the split that follows is a separate component because that is also what the model
 * performs. A backend is free to fuse projection, split and activation back into one kernel —
 * Phi3's GPU path does exactly that — and the program still describes both steps.
 *
 * <p>The rotary layout is NEOX_HALF, as Qwen2's is.
 *
 ...
 * over.
 */
public final class Phi3ProgramDescription {
    ...
    /** The fused QKV projection's output, which {@code SplitFusedQkv} reads. */
    static final String QKV = "qkv";

    /** The fused gate/up projection's output, which {@code SplitGateUp} reads. */
    static final String HB_GATE_UP = "hbGateUp";
    ...
    private Phi3ProgramDescription() {}
    ...
    private static ProgramComponent layerComponent(
        ...
        int kvWidth = config.numberOfKeyValueHeads() * headSize;
                ...
                List.of(
                        leaf(
                                ...
                                new RmsNorm(
                                        ...
                                        config.rmsNormEps(),
                                        DataType.F32)),
                        // One matrix, one multiply — the model fuses the three projections.
                        leaf(
                                "qkv" + suffix,
                                new MatVec(
                                        weight(TensorRole.ATTENTION_QKV),
                                        activation(XB),
                                        activation(QKV),
                                        dim + 2 * kvWidth,
                                        dim,
                        ...
                        leaf(
                                ...
                                new RmsNorm(
                                        ...
                                        config.rmsNormEps(),
                                        DataType.F32)),
                        // The second fused projection: gate and up end to end in one buffer.
                        leaf(
                                "gateUp" + suffix,
                                new MatVec(
                                        weight(TensorRole.FFN_GATE_UP),
                                        activation(XB),
                                        activation(HB_GATE_UP),
                                        2 * config.hiddenDim(),
                                        dim,
                                        weightDataType)),
                        leaf(
