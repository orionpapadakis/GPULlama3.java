...
public class Phi3TornadoVMLayerPlannerQ8_0 extends TornadoVMQ8_0LayerPlanner<Phi3State, Phi3Configuration, Phi3TornadoWeightsQ8_0> {
     ...
     */
    public Phi3TornadoVMLayerPlannerQ8_0(Phi3State state, Model model) {
        super(state, model);
    }
    ...
    public Tuple2<List<ImmutableTaskGraph>, GridScheduler> setupTornadoForwardPlanLayered() {
        ...
        for (int layerIndex = 0; layerIndex < config.numberOfLayers(); layerIndex++) {
            ...
            unifiedLayer.task("reductionsOneBlock" , TransformerComputeKernelsLayered::reductionOneBlockWithLayer, context, state.temp,
                    ...
                    .task("qkvmatmul", TransformerComputeKernelsLayered::matrixVectorGeneric, context, state.wrapXb, state.wrapQkv,
                            weights.wqkvLayered[layerIndex].getQuants(), weights.wqkvLayered[layerIndex].getScales(), config.dim(), opSize, LOCAL_WORK_GROUP_SIZE_ALLOC)
                    ...
                    .task("wGateUp", TransformerComputeKernelsLayered::matrixVectorGeneric, context,
                            state.wrapXb,   state.wrapHb, weights.wUpLayered[layerIndex].getQuants(), weights.wUpLayered[layerIndex].getScales(), config.dim(), 2 * config.hiddenDim(),  LOCAL_WORK_GROUP_SIZE_ALLOC)
                    ...
                    .task("wDown", TransformerComputeKernelsLayered::matrixVectorGenericWithResidual, context,
                            state.wrapHbU, state.wrapX, weights.wDownLayered[layerIndex].getQuants(), weights.wDownLayered[layerIndex].getScales(), config.hiddenDim(), config.dim(),  LOCAL_WORK_GROUP_SIZE_ALLOC)
                    .persistOnDevice(
                            state.wrapX
                    );
        ...
        TaskGraph logits = new TaskGraph("logits")
                ...
                .transferToDevice(DataTransferMode.FIRST_EXECUTION,
                        context,
                        state.wrapLogits,
                        weights.wclsHalfFloat.getQuants(),
                        weights.wclsHalfFloat.getScales(),
