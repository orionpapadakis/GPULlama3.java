...
// @formatter:off
    ...
    public class TornadoVMLayerPlanner<S extends State, C extends Configuration, W extends FP16Weights> implements TornadoVMGenericLayerPlanner{
        ...
        activation.updateGridScheduler(tornadoForwardScheduler);

         LlamaFP16FFNLayers llamaFFNLayers = new LlamaFP16FFNLayers("",state, weights, config) ;
         taskGraphs.addAll(llamaFFNLayers.getFfnLayerTaskGraphs());
        llamaFFNLayers.updateGridScheduler(tornadoForwardScheduler);

        LogitsFP16Layer logitsLayer = new LogitsFP16Layer("logits", state, weights, config, llamaFFNLayers.getLastTaskGraphID());
        taskGraphs.add(logitsLayer.getTaskGraph().snapshot());
        logitsLayer.updateGridScheduler(tornadoForwardScheduler);
